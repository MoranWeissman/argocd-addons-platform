...
function getStatusColor(status: string): { dot: string; bg: string; text: string } {
  ...
  if (['healthy', 'synced', 'connected', 'healthy'].includes(s)) {
  ...
  if (['progressing', 'outofsync', 'warning', 'missing', 'missing_in_argocd'].includes(s)) {
    return { dot: 'bg-yellow-500', bg: 'bg-yellow-50 dark:bg-yellow-900/30', text: 'text-yellow-700 dark:text-yellow-400' };
  }
